...
// ==================================================
// Magentic Ledger（台帳）管理テスト
// ==================================================

describe("AgentManager — Magentic Ledger 管理", () => {
  let manager: AgentManager;
  let config: AppConfig;
  ...
  const sampleTaskLedger: Omit<TaskLedger, "updatedAt"> = {
    facts: {
      given: ["プロジェクトは TypeScript 製"],
      toInvestigate: ["既存の API 仕様"],
      toDerive: ["最適なアーキテクチャ"],
      assumptions: ["Node.js 18+ を使用"],
    },
    plan: ["ステップ1: 調査", "ステップ2: 実装", "ステップ3: テスト"],
  };

  const sampleProgressLedger: Omit<ProgressLedger, "updatedAt"> = {
    isRequestSatisfied: { reason: "まだ実装が途中", answer: false },
    isInLoop: { reason: "新しいステップに進んでいる", answer: false },
    isProgressBeingMade: { reason: "ステップ1が完了した", answer: true },
    nextAction: { reason: "次は実装フェーズ", answer: "impl-code を起動する" },
    instruction: { reason: "認証モジュールが必要", answer: "src/auth/login.ts を実装せよ" },
    iteration: 1,
  };

  it("updateTaskLedger でタスク台帳が保存される", () => {
    const group = manager.createGroup("Magentic", "magentic");
    manager.updateTaskLedger(group.id, sampleTaskLedger);

    const ledger = manager.getLedger(group.id);
    expect(ledger).toBeDefined();
    expect(ledger!.groupId).toBe(group.id);
    expect(ledger!.taskLedger).toBeDefined();
    expect(ledger!.taskLedger!.facts.given).toEqual(["プロジェクトは TypeScript 製"]);
    expect(ledger!.taskLedger!.plan).toHaveLength(3);
    expect(ledger!.taskLedger!.updatedAt).toBeDefined();
  });

  it("updateTaskLedger で group:ledger_updated イベントが発行される", () => {
    const group = manager.createGroup("Magentic", "magentic");
    const handler = vi.fn();
    ...
    expect(emitted.groupId).toBe(group.id);
    expect(emitted.taskLedger).toBeDefined();
  });

  it("updateProgressLedger で進捗台帳が保存される", () => {
    const group = manager.createGroup("Magentic", "magentic");
    manager.updateProgressLedger(group.id, sampleProgressLedger);

    const ledger = manager.getLedger(group.id);
    expect(ledger).toBeDefined();
    expect(ledger!.progressLedger).toBeDefined();
    expect(ledger!.progressLedger!.isRequestSatisfied.answer).toBe(false);
    expect(ledger!.progressLedger!.isProgressBeingMade.answer).toBe(true);
    expect(ledger!.progressLedger!.iteration).toBe(1);
    expect(ledger!.progressLedger!.updatedAt).toBeDefined();
  });

  it("updateProgressLedger で group:ledger_updated イベントが発行される", () => {
    const group = manager.createGroup("Magentic", "magentic");
    const handler = vi.fn();
    manager.on("group:ledger_updated", handler);
    ...
    const emitted: MagenticLedger = handler.mock.calls[0][0];
    expect(emitted.groupId).toBe(group.id);
    expect(emitted.progressLedger).toBeDefined();
  });

  it("updateProgressLedger が progressHistory に過去エントリを追記する", () => {
    const group = manager.createGroup("Magentic", "magentic");

    manager.updateProgressLedger(group.id, { ...sampleProgressLedger, iteration: 1 });
    manager.updateProgressLedger(group.id, { ...sampleProgressLedger, iteration: 2 });
    manager.updateProgressLedger(group.id, { ...sampleProgressLedger, iteration: 3 });

    const ledger = manager.getLedger(group.id);
    expect(ledger!.progressLedger!.iteration).toBe(3);
    expect(ledger!.progressHistory).toHaveLength(2);
    expect(ledger!.progressHistory[0].iteration).toBe(1);
    expect(ledger!.progressHistory[1].iteration).toBe(2);
  });

  it("progressHistory は最大 20 件で古いものが切り捨てられる", () => {
    const group = manager.createGroup("Magentic", "magentic");

    for (let i = 1; i <= 25; i++) {
      manager.updateProgressLedger(group.id, { ...sampleProgressLedger, iteration: i });
    }

    const ledger = manager.getLedger(group.id);
    expect(ledger!.progressLedger!.iteration).toBe(25);
    expect(ledger!.progressHistory).toHaveLength(20);
    expect(ledger!.progressHistory[0].iteration).toBe(5);
    expect(ledger!.progressHistory[19].iteration).toBe(24);
  });

  it("getLedger は未登録のグループでは undefined を返す", () => {
    expect(manager.getLedger("nonexistent")).toBeUndefined();
  });

  it("deleteGroup 時に ledger もクリーンアップされる", () => {
    const group = manager.createGroup("Magentic", "magentic");
    manager.updateTaskLedger(group.id, sampleTaskLedger);
    manager.updateProgressLedger(group.id, sampleProgressLedger);
    expect(manager.getLedger(group.id)).toBeDefined();

    manager.deleteGroup(group.id);

    expect(manager.getLedger(group.id)).toBeUndefined();
  });

  it("存在しないグループへの updateTaskLedger はエラーを投げる", () => {
    expect(() => manager.updateTaskLedger("nonexistent", sampleTaskLedger)).toThrow();
  });

  it("存在しないグループへの updateProgressLedger はエラーを投げる", () => {
    expect(() => manager.updateProgressLedger("nonexistent", sampleProgressLedger)).toThrow();
  });
  ...
  it("タスク台帳を上書き更新できる", () => {
    ...
    manager.updateTaskLedger(group.id, updatedData);

    const ledger = manager.getLedger(group.id);
    expect(ledger!.taskLedger!.facts.given).toEqual(["更新後の事実"]);
    expect(ledger!.taskLedger!.plan).toEqual(["新しい計画ステップ"]);
  });
});
