...
export class AgentManager extends EventEmitter {
  ...
  /** Magentic 台帳: groupId → MagenticLedger */
  private ledgers: Map<string, MagenticLedger>;
  ...
  // ==================================================
  // Magentic Ledger（台帳）管理
  // ==================================================

  private static readonly MAX_PROGRESS_HISTORY = 20;

  /** タスク台帳を更新する */
  updateTaskLedger(
    groupId: string,
    data: Omit<TaskLedger, "updatedAt">,
  ): void {
    const group = this.groups.get(groupId);
    if (!group) {
      throw new Error(`グループが見つかりません: ${groupId}`);
    ...
    let ledger = this.ledgers.get(groupId);
    if (!ledger) {
      ledger = { groupId, progressHistory: [] };
      this.ledgers.set(groupId, ledger);
    }

    ledger.taskLedger = { ...data, updatedAt: new Date().toISOString() };
    this.emit("group:ledger_updated", ledger);
  }

  /** 進捗台帳を更新する */
  updateProgressLedger(
    groupId: string,
    data: Omit<ProgressLedger, "updatedAt">,
  ): void {
    const group = this.groups.get(groupId);
    if (!group) {
      throw new Error(`グループが見つかりません: ${groupId}`);
    ...
    if (ledger.progressLedger) {
      ledger.progressHistory.push(ledger.progressLedger);
      if (ledger.progressHistory.length > AgentManager.MAX_PROGRESS_HISTORY) {
        ledger.progressHistory = ledger.progressHistory.slice(
          ledger.progressHistory.length - AgentManager.MAX_PROGRESS_HISTORY,
        );
      }
    }

    ledger.progressLedger = { ...data, updatedAt: new Date().toISOString() };
    this.emit("group:ledger_updated", ledger);
  }

  /** 台帳を取得する */
  getLedger(groupId: string): MagenticLedger | undefined {
    return this.ledgers.get(groupId);
  }
  ...
  startAgent(
    ...
    const fullPrompt =
      ...
      this.resolveFullPrompt(role, agentId, groupId, prompt);
    const timeout_ms =
  ...
  buildMagenticOrchestratorPrompt(
    ...
    const toolGuideBlock = [
      ...
      "- **update_ledger**: 台帳（Ledger）を更新する。思考過程を構造化して記録するために使用する。",
      "",
      "【台帳（Ledger）の記録ルール — 必ず守ること】",
      "",
      "人間がダッシュボードであなたの思考過程を追跡できるよう、以下のタイミングで update_ledger を呼び出してください。",
      "",
      "1. **タスク分析完了時**: `update_ledger(type: \"task\")` を呼び出し、以下を記録する:",
      "   - facts.given: 与えられた事実（ユーザーの指示や前提条件）",
      "   - facts.toInvestigate: 調査が必要な事項",
      "   - facts.toDerive: 導出すべき事項",
      "   - facts.assumptions: 推測・仮定",
      "   - plan: 実行計画のステップ一覧",
      "",
      "2. **各反復の開始時**: `update_ledger(type: \"progress\")` を呼び出し、以下の 5 項目を判定・記録する:",
      "   - isRequestSatisfied: { reason: \"判定理由\", answer: true/false } — リクエストは満たされたか",
      "   - isInLoop: { reason: \"判定理由\", answer: true/false } — ループに陥っていないか",
      "   - isProgressBeingMade: { reason: \"判定理由\", answer: true/false } — 進捗が出ているか",
      "   - nextAction: { reason: \"判定理由\", answer: \"次のアクションの説明\" } — 次に何をするか",
      "   - instruction: { reason: \"判定理由\", answer: \"サブ Agent への具体的指示\" } — サブ Agent にどう指示するか",
      "   - iteration: 現在の反復番号",
      "",
      "3. **リプラン時**: 計画を見直した場合は `update_ledger(type: \"task\")` で計画を更新する。",
      "",
      "---",
    ].join("\n");
  ...
  reportResult(
    ...
    // Magentic Orchestrator の場合、最終 ledger スナップショットを保存
    const group = this.groups.get(agent.groupId);
    if (
      group?.mode === "magentic" &&
      group.orchestratorAgentId === agentId
    ) {
      const ledger = this.ledgers.get(agent.groupId);
      if (ledger) {
        result.finalLedger = JSON.parse(JSON.stringify(ledger));
      }
    }
  ...
  /**
  /**
   * Magentic Orchestrator の場合は専用プロンプト、それ以外は汎用プロンプトを返す。
   */
  private resolveFullPrompt(
    role: RoleDefinition,
    agentId: string,
    groupId: string,
    userPrompt: string,
  ): string {
    const magenticConfig = this.magenticConfigs.get(groupId);
    const group = this.groups.get(groupId);
    if (group?.mode === "magentic" && magenticConfig && !group.orchestratorAgentId) {
      return this.buildMagenticOrchestratorPrompt(role, agentId, groupId, magenticConfig);
    }
    return this.buildFullPrompt(role, agentId, groupId, userPrompt);
  }
