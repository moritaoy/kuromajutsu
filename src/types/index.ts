...
/** Magentic 台帳（タスク台帳 + 進捗台帳 + 履歴） */
export interface MagenticLedger {
  /** 対象グループ ID */
  groupId: string;
  /** タスク台帳（最新） */
  ...
  /** 過去の進捗台帳履歴 */
  progressHistory: ProgressLedger[];
}
...
export interface AgentResult {
  ...
  rawOutput?: string;
  /** Magentic Orchestrator 完了時の台帳スナップショット */
  finalLedger?: MagenticLedger;
  /** 完了時刻（ISO 8601） */
  timestamp: string;
}
...
export type ServerEvent =
  ...
  | { type: "group:ledger_updated"; data: MagenticLedger }
